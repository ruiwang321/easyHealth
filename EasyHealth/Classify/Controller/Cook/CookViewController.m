...
#import "CookListModel.h"
#import "CookListCollectionViewCell.h"

#import "ShowDetailViewController.h"

@interface CookViewController () <UITableViewDataSource, UITableViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout> {
...
- (void)viewDidLoad {
    ...
    self.navigationItem.titleView = [self setTitleColor:@"菜谱" withFrame:CGRectMake(0, 0, 100, 40)];
...
- (NSArray *)classifyModelsList {
    if (_classifyModelsList == nil) {
        _classifyModelsList = [CookClassifyModel classifyModelsList];
    }
...
- (void)getDataWithId:(NSString *)Id {
    ...
    [manager GET:API_COOK_LIST parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        [self.listDataArray removeAllObjects];
        for (NSDictionary *dic in responseObject[@"tngou"]) {
            CookListModel *model = [[CookListModel alloc] init];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    CookListCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"listCell" forIndexPath:indexPath];
    ...
    [cell.iconView sd_setImageWithURL:[NSURL URLWithString:url]  placeholderImage:[UIImage imageNamed:@"zhanwei_v"]];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CookListModel *model = self.listDataArray[indexPath.row];
